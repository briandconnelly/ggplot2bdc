...
midwest$inmetro <- factor(midwest$inmetro, levels = c(0, 1), labels = c("Rural", "Metro"))

ggplot(data = midwest, aes(x = inmetro, y = percbelowpoverty, color = state, shape = state, group = state)) +
    stat_summary(fun.y = "mean", geom = "line") +
    stat_summary(fun.y = "mean", geom = "point") +
    scale_color_hue(name = "State") +
    scale_shape_discrete(name = "State") +
    labs(x = "Environment", y = "Percent of Population",
...
txcities <- txhousing %>%
    filter(city %in% c("Austin", "Dallas", "El Paso", "Fort Worth", "Houston", "San Antonio"))

ggplot(data = txcities, aes(x = year, y = median / 1000)) +
    facet_grid(city ~ .) +
    stat_summary(fun.data = "mean_cl_boot", geom = "ribbon", color = NA, alpha = 0.3) +
    stat_summary(fun.y = "mean", geom = "line") +
...
```{r theme_bdc_lattice_population with facets}
library(dplyr)
